...
import torch
from fedml_api.standalone.domain_generalization.utils.finch import FINCH
import numpy as np
...
class FPL(FederatedModel):
    ...
    def proto_aggregation(self, local_protos_list):
        ...
        for [label, proto_list] in agg_protos_label.items():
            if len(proto_list) > 1:
                proto_list = [
                    item.squeeze(0).detach().cpu().numpy().reshape(-1)
    ...
    def hierarchical_info_loss(
        self, f_now, label, all_f, mean_f, all_global_protos_keys
    ):
    ...
    def _train_net(self, index, net, train_loader):
        ...
        for iter in iterator:
            ...
            for batch_idx, (images, labels) in enumerate(train_loader):
                ...
                loss.backward()
                iterator.desc = "Local Pariticipant %d CE = %0.3f,InfoNCE = %0.3f" % (
                    index,
                    lossCE,
                    loss_InfoNCE,
                )
