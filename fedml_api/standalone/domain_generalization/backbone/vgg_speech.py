    ...
    for v in cfg:
        if v == "M":
            ...
            if batch_norm:
                layers += [
                    conv2d,
                    nn.BatchNorm2d(v, track_running_stats=False),
...
def vgg11(nclasses: int, input_channels: int = 1):
    return VGG("VGG11", input_channels, nclasses)
...
if __name__ == "__main__":
    ...
    print(
        "num parameters:",
        sum(p.numel() for p in bcresnet.parameters() if p.requires_grad),
    )
